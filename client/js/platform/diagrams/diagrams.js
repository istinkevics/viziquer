...
Template.diagramsRibbon.events({
    ...
    'click #mysql-settings': function (e, templ) {
        $('#mysql-settings-form').modal("show");
    },
...
Template.mysqlSettings.helpers({
	msg() {
        return Session.get('msg');
    },
    project() {
...
Template.mysqlSettings.events({

    'click #ok-mysql-settings': function (e, templ) {
        var list = {
            projectId: Session.get("activeProject"),
            versionId: Session.get("versionId"),
        ...
        Utilities.callMeteorMethod('updateProjectMySQLSettings', list);
    },

    'click #cancel-mysql-settings': function (e, templ) {
        var proj = Projects.findOne({ _id: Session.get('activeProject') });
        if (proj) {
            $('#db-hostname').val(proj.dbHostname);
            $('#db-port').val(proj.dbPort);
            $('#db-username').val(proj.dbUsername);
            $('#db-password').val(proj.dbPassword);
            $('#db-name').val(proj.dbName);
        }
    },

    "click #test-connection": function (e) {
        var list = {
            projectId: Session.get("activeProject"),
            versionId: Session.get("versionId"),
            dbHostname: $("#db-hostname").val(),
        ...
        Utilities.callMeteorMethod('testMySQLProjectEndPoint', list, function (res) {
            let className = 'danger';
            let text = 'Connection is not ok';
            ...
            const msg = { text, class: className };
